...
/** Lazy-load dash.js on first use and return a Promise that resolves when ready */
let _dashjsPromise = null;
function loadDashJs() {
  if (_dashjsPromise) return _dashjsPromise;
  _dashjsPromise = new Promise((resolve, reject) => {
    /* Already loaded */
    if (typeof window.dashjs !== "undefined") { resolve(); return; }
    const s = document.createElement("script");
    /* Try official CDN first, fall back to jsDelivr */
    ...
    s.onerror = () => {
      ...
      s2.src = "https://cdn.jsdelivr.net/npm/dashjs@4.7.4/dist/dash.all.min.js";
      s2.crossOrigin = "anonymous";
      s2.onload  = () => resolve();
      s2.onerror = () => reject(new Error("dash.js failed to load"));
...
/** Play a DASH (.mpd) stream with optional Clearkey DRM */
...
function _startDashPlayer(ch) {
  dashPlayer = window.dashjs.MediaPlayer().create();
  dashPlayer.initialize(video, null, false);
  ...
  /* Route all DASH requests through our proxy.
     dash.js v4 removed 'extend/RequestModifier' — use addRequestInterceptor instead.
     This intercepts EVERY request (MPD, init segments, media segments)
     and rewrites external URLs to go through our proxy with custom headers. */
  if (typeof dashPlayer.addRequestInterceptor === "function") {
    dashPlayer.addRequestInterceptor(function (request) {
      if (request.url && !request.url.startsWith(window.location.origin)) {
        request.url = proxiedStreamUrl(request.url, ch.headers);
      }
      return Promise.resolve(request);
    });
  } else if (dashPlayer.extend) {
    /* Fallback for older dash.js versions */
    dashPlayer.extend("RequestModifier", function () {
      return {
        modifyRequestURL: function (url) {
          if (url.startsWith(window.location.origin)) return url;
          return proxiedStreamUrl(url, ch.headers);
        },
  ...
  if (ch.drm && ch.drm.type === "org.w3.clearkey" && ch.drm.key) {
    try {
      const keyData = JSON.parse(ch.drm.key);
      dashPlayer.setProtectionData({
        "org.w3.clearkey": { clearkeys: parseClearkeys(keyData) },
      });
      console.log("[DRM] Clearkey protection data set");
    } catch (e) {
      console.warn("Failed to parse Clearkey DRM data:", e);
    }
  }

  const mpdUrl = proxiedStreamUrl(ch.url, ch.headers);
  dashPlayer.attachSource(mpdUrl);

  dashPlayer.on(window.dashjs.MediaPlayer.events.CAN_PLAY, () => {
    showLoading(false);
    video.play().catch(() => {});
  });
