...
function proxiedStreamUrl(url, headers) {
  ...
  const endpoint = isDirectIP ? "/api/hlsnode" : "/api/hls";
  ...
  for (let raw of lines) {
    ...
    if (isStreamUrl(url)) {
      ...
      const drm = pendingProps.drmType
        ? { type: pendingProps.drmType, key: pendingProps.drmKey || "" }
        : null;
...
/**
 * Parse Clearkey JSON (Kodi format) into {kid: key} map for dash.js
 * Input: {"keys":[{"kty":"oct","key":"base64url","kid":"base64url"}],"type":"temporary"}
 */
function parseClearkeys(keyData) {
  const map = {};
  if (keyData && keyData.keys) {
    for (const k of keyData.keys) {
      if (k.kid && k.key) { map[k.kid] = k.key; }
    }
  }
  return map;
}
