...
function _startShakaPlayer(ch) {
  ...
  dashPlayer.attach(video).then(() => {
    ...
    const code = err?.code     ?? "?";
    let   msg  = `Stream error [${cat}:${code}]`;
    if (cat === 1) msg += " — Network error. The CDN may be geo-blocked or offline.";
    else if (cat === 7) msg += " — DRM error. The decryption key may be invalid.";
    else if (cat === 4) msg += " — Manifest error. The MPD could not be parsed.";
    else msg += " — The stream may be offline or expired.";
    showError(msg);
  });
}
