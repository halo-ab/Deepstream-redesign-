...
/**
 * Smart stream URL classifier.
 * Returns the best initial play strategy based on the URL.
 *   "direct"  → try without proxy first (HTTPS CDN streams, CloudFront, etc.)
 *   "proxy"   → go straight to proxy (FanCode needs spoofed Referer)
 */
function classifyUrl(url) {
  try {
    ...
    // FanCode always needs the proxy (requires spoofed Referer header)
    if (host.includes("fancode") || host.includes("media-cdn"))
      return "proxy";

    // Geo-locked direct-IP IPTV on HTTP → try direct first (user's IP)
    // If on HTTPS site mixed-content will block it → proxy fallback kicks in
    return "direct";
  } catch {
    return "proxy";
  }
...
/** Detect link type for status display */
function detectLinkType(url) {
  if (/\.m3u8?([\?#]|$)/i.test(url)) return "HLS (.m3u8)";
  if (/\.m3([\?#]|$)/i.test(url))    return "HLS (.m3)";
  if (/\.ts([\?#]|$)/i.test(url))    return "MPEG-TS (.ts)";
  if (/\.mp4([\?#]|$)/i.test(url))   return "MP4";
  return "Stream";
}

// Play stage tracker per channel
let _playStage = "idle"; // "direct" | "proxy" | "done"
let _playUrl   = "";
...
function playChannel(ch) {
  ...
  _playUrl   = ch.url;
  _playStage = classifyUrl(ch.url); // start at "direct" or "proxy"

  startStage(ch.url, _playStage);
}

/**
 * Attempt playback at a given stage.
 * @param {string} rawUrl   - original stream URL from playlist
 * @param {"direct"|"proxy"} stage
 */
function startStage(rawUrl, stage) {
  destroyPlayer();
  networkRetries = 0;
  showLoading(true);
  hideError();
  ...
  const playUrl = stage === "proxy" ? proxiedStreamUrl(rawUrl) : rawUrl;
  const linkType = detectLinkType(rawUrl);

  console.log(`[DeepStream] ${stage.toUpperCase()} attempt | ${linkType} | ${rawUrl}`);
  ...
  if (Hls.isSupported()) {
    ...
    hls = new Hls({
      ...
      manifestLoadingTimeOut:  stage === "direct" ? 8000 : 20000, // fast fail on direct
      manifestLoadingMaxRetry: stage === "direct" ? 1    : 4,
      levelLoadingTimeOut:     stage === "direct" ? 8000 : 20000,
      fragLoadingTimeOut:      30000,
      fragLoadingMaxRetry:     stage === "direct" ? 1    : 6,
      xhrSetup(xhr) {
    ...
    hls.on(Hls.Events.ERROR, (_evt, data) => {
      ...
      if (stage === "direct") {
        // Direct failed → automatically fall back to proxy
        console.log("[DeepStream] Direct failed, retrying via proxy…");
        _playStage = "proxy";
        startStage(rawUrl, "proxy");
      } else {
        // Proxy also failed
        showLoading(false);
    ...
    video.addEventListener("error", () => {
      if (stage === "direct") {
        console.log("[DeepStream] Native direct failed, retrying via proxy…");
        startStage(rawUrl, "proxy");
      } else {
        showLoading(false);
        showError("Native HLS playback failed. Stream may be offline or geo-blocked.");
      }
    }, { once: true });
...
function handleFatalError(data) {
  ...
  showError("Stream unavailable — offline, expired, or geo-blocked in your region.");
}
