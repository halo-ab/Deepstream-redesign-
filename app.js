...
function playChannel(ch) {
  ...
  if (Hls.isSupported()) {
    ...
    hls.on(Hls.Events.MANIFEST_PARSED, () => {
      showLoading(false);
      video.play().catch(() => {});
      /* Expose quality levels to the UI layer */
      window.__hlsInstance = hls;
      document.dispatchEvent(new CustomEvent("ds:qualitylevels", {
        detail: { levels: hls.levels, current: hls.currentLevel }
      }));
    });

    /* Update quality indicator when level auto-switches */
    hls.on(Hls.Events.LEVEL_SWITCHED, (_e, data) => {
      document.dispatchEvent(new CustomEvent("ds:levelswitched", {
...
function destroyPlayer() {
  ...
  document.dispatchEvent(new CustomEvent("ds:qualityclear"));
