...
function _startShakaPlayer(ch) {
  ...
  dashPlayer.attach(video).then(() => {
    ...
    /* ── Request filter: proxy ALL external requests + add custom headers ── */
    dashPlayer.getNetworkingEngine().registerRequestFilter(function (type, request) {
      request.uris = request.uris.map((uri) => {
        if (uri.startsWith(window.location.origin)) return uri;
    ...
    if (ch.drm && ch.drm.type === "org.w3.clearkey" && ch.drm.key) {
      try {
        ...
        const clearKeys  = parseClearkeys(keyData);  /* {kidHex: keyHex} */
        shakaConfig.drm  = { clearKeys };
        console.log("[DRM] Shaka ClearKey configured:", Object.keys(clearKeys));
      } catch (e) {
        console.warn("Failed to parse Clearkey DRM data:", e);
      }
    }

    dashPlayer.configure(shakaConfig);
    ...
    dashPlayer.addEventListener("error", (e) => {
      ...
      if (networkRetries < 4) {
        networkRetries++;
        dashPlayer.load(proxiedStreamUrl(ch.url, ch.headers))
          .catch(() => {});
      } else {
        showLoading(false);
    ...
    showLoading(false);
    showError("DASH stream failed to load. The stream may be offline or unavailable.");
  });
}
