...
/**
 * Build proxy URL with optional custom headers (for HLS streams).
 * @param {string} url
...
/**
 * Build proxy URL for DASH/MPD streams — uses dedicated /api/dash endpoint.
 * @param {string} url
 * @param {{userAgent?: string, referer?: string}} [headers]
 */
function proxiedDashUrl(url, headers) {
  let proxyUrl = `${window.location.origin}/api/dash?url=${encodeURIComponent(url)}`;
  if (headers?.userAgent) proxyUrl += `&ua=${encodeURIComponent(headers.userAgent)}`;
  if (headers?.referer)   proxyUrl += `&referer=${encodeURIComponent(headers.referer)}`;
  return proxyUrl;
}
...
function _startShakaPlayer(ch) {
  ...
  dashPlayer.attach(video).then(() => {
    ...
    /* Route ALL requests through our dedicated /api/dash proxy.
       This proxy runs in Mumbai (bom1) region — same country as
       FanCode's CDN — and adds CORS headers for the browser.
       No HLS logic, pure binary passthrough. */
    net.clearAllRequestFilters();
    net.registerRequestFilter((type, request) => {
      request.uris = request.uris.map((uri) => {
        if (!uri || uri.startsWith(window.location.origin)) return uri;
        return proxiedDashUrl(uri, ch.headers);
      });
    });

    const mpdUrl = proxiedDashUrl(ch.url, ch.headers);
    console.log("[Shaka] Loading via /api/dash:", mpdUrl);
    return dashPlayer.load(mpdUrl);


  }).then(() => {
    console.log("[Shaka] DASH stream loaded successfully");
    showLoading(false);
    ...
    console.error("[Shaka] DASH load failed:", err);
    showLoading(false);
