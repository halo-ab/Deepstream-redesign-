...
function _startShakaPlayer(ch) {
  ...
  dashPlayer.attach(video).then(() => {
    ...
    /* For live streams, seek to the live edge so playback starts immediately
       instead of buffering from a massive presentation time offset */
    if (dashPlayer.isLive && dashPlayer.isLive()) {
      const seekRange = dashPlayer.seekRange();
      if (seekRange && seekRange.end) {
        console.log("[Shaka] Live stream detected. Seeking to live edge:", seekRange.end);
