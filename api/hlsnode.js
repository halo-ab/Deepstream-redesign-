...
function makeProxyUrl(origin, target) {
...
function rewriteManifest(text, sourceUrl, proxyOrigin) {
  ...
  let out = text
    .replace(/URI="([^"]+)"/gi, (_, u) => `URI="${makeProxyUrl(proxyOrigin, resolveUrl(base, u))}"`)
    .replace(/URI='([^']+)'/gi, (_, u) => `URI='${makeProxyUrl(proxyOrigin, resolveUrl(base, u))}'`);

  return out.split(/\r?\n/).map((line) => {
    const t = line.trimEnd();
    if (!t || t.startsWith("#")) return t;
    return makeProxyUrl(proxyOrigin, resolveUrl(base, t));
  }).join("\n");
}
...
export default async function handler(req, res) {
  ...
  /* Extra headers (forward Range if present) */
  const extra = {};
