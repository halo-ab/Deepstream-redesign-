...
function looksLikeManifest(url, ct) {
  ...
  if (ct && /mpegurl|m3u|dash\+xml/i.test(ct)) return true;
  if (/\/(video|stream|live|hls|channel|play)(\/|\.m3|$)/i.test(url)) return true;
  return false;
}

function isDashManifest(url, ct) {
  if (/\.mpd([\?#]|$)/i.test(url)) return true;
  if (ct && /dash\+xml/i.test(ct)) return true;
  return false;
}
...
export default async function handler(req, res) {
  ...
  if (looksLikeManifest(target, ct)) {
    ...
    /* DASH/MPD — pass through as-is with CORS.
       dash.js intercepts segment requests client-side via addRequestInterceptor. */
    if (isDashManifest(target, ct) || trimmed.startsWith("<?xml") || trimmed.startsWith("<MPD")) {
